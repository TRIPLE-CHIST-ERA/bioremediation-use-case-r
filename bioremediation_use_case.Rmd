...
```{r step-1-get-pollutants, message=FALSE, echo=FALSE}
...
# query_wikidata <- load_query_from_file(
#   query_file_wikidata
# )
...
```{r step-2-run-query, echo=FALSE, message=FALSE}
...
knitr::kable(
  ...
  col.names = c(
    paste0("Chemical similar to ", pollutant_name, " (as CHEBI numbers)"),
    "Similarity score"
  ),
  caption = paste0(
...
```{r step-3-run-query, echo=FALSE, message=FALSE}
...
```{r step-4-run-query, message=FALSE, echo=FALSE}
...
unique_taxa <- oma_taxa |>
  dplyr::select("taxon_sci_name", "oma_link") |>
  dplyr::distinct()
```

This query identifies **`r nrow(unique_taxa)`** species with potential for
bioremediation of **`r pollutant_name`**. As an example, here are the first
10 of them:

```{r step-4-display-table, echo=FALSE}

unique_taxa_head <- unique_taxa |>
  dplyr::slice_head(n = 10) |>
  dplyr::rowwise() |>
  dplyr::mutate(
    taxon_link = paste0(
      "[", taxon_sci_name, "](",
      stringr::str_replace(oma_link, "<(.*)>", "\\1"),
      ")"
    ),
    .keep = "none"
  )
...
Graphical representation of the SPARQL queries ran for the first 3 steps of
the bioremediation use case.


```{r build-network, echo=FALSE, message=FALSE, warning=FALSE}
...
# query_oma_file <- "../sparqlr.git/example_queries/construct_4_oma.rq"
...
# query_oma <- load_query_from_file(query_oma_file)
# prefixes_oma <- load_prefixes_from_file(query_oma_file)


constr_1 <- sparql_construct(
  endpoint = endpoint_wikidata,
  query = query_wikidata,
  prefixes = prefixes_wikidata,
  iri_style = "short"
)
constr_2 <- sparql_construct(
  endpoint = endpoint_idsm,
  query = query_idsm,
  prefixes = prefixes_idsm,
  iri_style = "short"
)
constr_3 <- sparql_construct(
  endpoint = endpoint_uniprot,
  query = query_uniprot,
  prefixes = prefixes_uniprot,
  iri_style = "short",
  use_post = TRUE,
)
# OMA does not seem to support CONSTRUCT requests.
# constr_4 <- sparql_construct(
#   endpoint = endpoint_uniprot,
#   query = query_oma,
#   prefixes = prefixes_oma,
#   iri_style = "short",
#   use_post = TRUE,
# )
...
```{r print-network, echo=FALSE, message=FALSE}
