...
<!-- To run this file in VS code: Ctrl + Shift + k -->
...
library(rlang)
library(sparqlr)
source("utils.R")  # Load helper functions.
...
get_pollutants_from_solid_pod <- FALSE
if (get_pollutants_from_solid_pod) {
  ...
  # Retrieve pollutant data from a local file. Use when Solid Pod is offline.
  pollutants <- read.table(
    "output/pollutants_table.tsv", sep = "\t", header = TRUE
  )
}
...
readr::write_tsv(pollutants, "output/pollutants_table.tsv")
```
...
bacterial_taxa <- sparql_select(
  endpoint = endpoint_uniprot,
  query = query_uniprot_mnemonic
)
taxa_counts <- oma_taxa |>
  dplyr::semi_join(bacterial_taxa, by = "mnemonic") |>
...
# Save output to disk as TSV files.
readr::write_tsv(
  oma_taxa |>
    dplyr::semi_join(bacterial_taxa, by = "mnemonic") |>
    dplyr::select(
      "taxon_sci_name",	"taxon", "mnemonic", "ortholog_protein", "oma_link"
    ),
  "output/bioremediation_output_taxa_and_enzymes.tsv"
)
readr::write_tsv(taxa_counts, "output/bioremediation_output_taxa_counts.tsv")
```
...
wikidata_chebi_info <- sparql_select(
  endpoint = endpoint_wikidata,
...
wikidata_uniprot_info <- sparql_select(
  endpoint = endpoint_wikidata,
